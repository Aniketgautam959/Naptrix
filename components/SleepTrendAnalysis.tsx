    ...
    const calculateTrends = () => {
      ...
      if (selectedPeriod === 'week') {
        ...
        weekEntries.forEach(([weekKey, weekRecords], index) => {
          const avgSleep =
            weekRecords.reduce((sum, r) => sum + r.amount, 0) /
            weekRecords.length;
          ...
          trends.push({
            period: new Date(weekKey).toLocaleDateString('en-US', {
              month: 'short',
              day: 'numeric',
            }),
        ...
        monthEntries.forEach(([monthKey, monthRecords], index) => {
          ...
          trends.push({
            period: new Date(monthKey + '-01').toLocaleDateString('en-US', {
              month: 'short',
              year: 'numeric',
            }),
            averageSleep: avgSleep,
    ...
    return (
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-8 text-center">
        <p className="text-sm text-slate-400">
          Add more records to see trend analysis
        </p>
      </div>
    );
  }
            ...
            <div className="flex-1 flex items-end space-x-2 relative">
              ...
              <div className="absolute inset-0 flex flex-col justify-between">
                {[0, 1, 2, 3, 4, 5].map(i => (
                  <div
                    key={i}
                    className="border-t border-slate-200 dark:border-slate-600 opacity-30"
                  ></div>
                ))}
              </div>
              ...
              {trendData.map((data, index) => {
                ...
                const barHeight = Math.max(
                  30,
                  (data.averageSleep / 10) * maxHeight
                );
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-2xl font-bold text-slate-800">
                    {(
                      trendData.reduce((sum, d) => sum + d.averageSleep, 0) /
                      trendData.length
                    ).toFixed(1)}
                    h
                  </div>
      ...
      <div className="space-y-4">
        ...
        {trendData.slice(-3).map((data, index) => (
          ...
          >
            <div className="flex items-center space-x-3">
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center border ${getTrendColor(data.trend)}`}
              >
              ...
              <div>
                ...
                <div className="text-sm text-slate-600">
                  {data.averageSleep.toFixed(1)}h average • Quality:{' '}
