...
const links = [
  { href: '/', label: 'Home', labelAuth: 'Dashboard' },
  ...
  return (
    <div className='hidden md:flex items-center gap-0.5'>
      {links.map((link) => {
        ...
        const label = link.href === '/' && isLoggedIn ? link.labelAuth : link.label;
      ...
      <a
        ...
        className='px-3 py-1.5 text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800/60 rounded-md transition-colors flex items-center gap-1.5'
      >
